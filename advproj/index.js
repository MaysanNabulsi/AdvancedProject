...
async function getJobListingWithWeather(jobListing) {
  const weatherData = await fetchWeatherData(jobListing.location);

  // Extract the relevant weather information from the API response
  const temperature = weatherData.main.temp;
  const weatherDescription = weatherData.weather[0].description;

  // Update the job listing object with weather information
  jobListing.temperature = temperature;
  jobListing.weatherDescription = weatherDescription;

  return jobListing;
}
app.get('/weather/:jobid', (req, res) => {
  const id = req.params.jobid;
  // Retrieve all job searches from the database
  db.query('SELECT location FROM job_listings WHERE jobid = ?', 
  [id],
  (err, result) => {
    if (err) {
      console.error('Error retrieving job listing: ', err);
      ...
      //res.json(result[0]);
      getJobListingWithWeather(result[0])
      .then(updatedJobListing => {
        res.json(updatedJobListing);
        // Display the updated job listing with weather information
      })
      .catch(error => {
...
//////////////////////////////////////////////////////////////////////// 
//feature #3
app.post('/jobs/:job_id/apply', (req, res) => {
  const job_id = req.params.job_id;
  const {  app_id, seeker_id,app_date, cover_letter } = req.body;
  ...
  db.query('INSERT INTO job_applications SET ?', application, (err, result) => {
    if (err) {
...
app.get('/jobs/:job_id/applications', (req, res) => {
  const job_id = req.params.job_id;

  // Retrieve the job applications for the specified job listing
  connection.query(
